...
class IDSAgentUI(QWidget):
    def __init__(self):
        ...
        # Tabs: Config first, Daemon/Logs second
    ...
    def init_config_tab(self):
        ...
        hub_url = self.config.get("hub_url", "http://127.0.0.1:5000/")
        device_id = self.config.get("device_id", "agent-123")
        api_key = self.config.get("api_key", "")
        self.hub_ip = QLineEdit(hub_url)
        ...
        # Assemble form
        form_layout.addRow("HUB URL:", self.hub_ip)
    ...
    def save_config(self):
        cfg = {
            "hub_url": self.hub_ip.text().strip(),
            "device_id": self.device_field.text().strip(),
            "api_key": self.apikey_field.text().strip(),
            "file_monitor": {
    ...
    def reload_config(self):
        self.config = self.load_config()
        self.hub_ip.setText(self.config.get("hub_url", "http://127.0.0.1:5000/"))
